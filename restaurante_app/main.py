...
def registrar_bebida(servicio: Restaurante) -> None:
    ...
    try:
        ...
        categoria = input("Categoria: ")
        precio = float(input("Precio: "))
        presentacion = input("Presentacion (ej. 350ml, mediana): ")
        envase = input("Envase (ej. lata, vaso, botella): ")
        bebida = Bebida(codigo, nombre, categoria, precio, presentacion, envase)
    except ValueError as error:
        print(f"Error: {error}")
        return
